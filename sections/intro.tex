...
We introduce \textbf{UMGS}, a UAV multispectral 3D Gaussian Splatting framework for spectral-index synthesis. UMGS formulates multispectral reconstruction as \emph{RGB-anchored geometry locking}: geometry, opacity, and Gaussian identity are learned once from RGB and then frozen during G/R/RE/NIR transfer. This design turns RGB into a stable geometric carrier, while each spectral band only learns appearance on the same splat support. We evaluate a primary realization and a close joint counterpart:
\begin{itemize}
    \item \textbf{UMGS-I}: the primary independent per-band transfer realization from a shared RGB anchor.
    \item \textbf{UMGS-J}: a joint multispectral realization with band-specific appearance banks in one unified checkpoint.
\end{itemize}
...
\begin{itemize}
    ...
    \item A primary independent-transfer realization (UMGS-I) and a close joint counterpart (UMGS-J) implementing the same lock.
