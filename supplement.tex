...
Table~\ref{tab:supp-umgsj-mms-scenes} provides scene-wise official common-mask comparison between UMGS-J-shim and MS-Splatting. This table expands the average values in the main text and makes the mixed scene-level behavior explicit.
...
The supplementary tables support three practical points used by the draft: (1) official external comparison is competitive but scene-dependent, (2) relative-depth trends are stable under threshold sweeps when support is preserved, and (3) UMGS-JR is an initialization ablation rather than a promoted branch.
